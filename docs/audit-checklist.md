...
| 6.7a | Flaky tests are diagnosed, not re-run until green | Fixed — the attachments spec matched the file name *anywhere on the page*, and both upload and delete raise a toast containing it. The row locator could resolve against the toast, so the buttons clicked were the toast's. It failed intermittently and always passed when re-run alone, once the toast had gone. Scoped to `main`; runtime fell from 6.0s to 1.8s because it had been waiting for toasts to clear. |
| 6.7b | Check-then-act races in specs | Fixed — the notification spec counted the row menus, then clicked `nth(1)`. Marking a row read refetches the list and can drop it from the Current tab, so the count was stale by the time the click landed: on CI it waited 30s for a second menu on a one-row list. It now re-resolves `first()` at each step. Any `if (await x.count() > n)` followed by a positional click has this shape. |
...
| 6.10 | Lint suppressions are justified and tracked, not silent | Fixed — and the count in this row was wrong: there were **11 sites, not three**, most predating the ESLint 10 upgrade. Three were real defects, now fixed by adjusting state during render instead of from an effect (search box syncing to the URL, the notifications page clamping past-the-end, custom-field column defaults, which are now derived rather than copied into state). Four more were copies of the same saved-view effect on pages that had never been migrated to `useSavedViewState`; those pages now use the shared hook. Five remain, each a genuine action rather than derived state (applying a saved view writes through the router; an async image fetch; two dialogs resetting on open), and each carries a specific reason. |
...
| 8.2 | Optimistic locking on concurrently edited records | Fixed — all ten versioned entities now round-trip `entityVersion` and refuse a stale edit with 409, proved per type by `OptimisticLockingIntegrationTest`. Five catalogue types (asset/application/certificate types, models, templates) had the `@Version` column but exposed it nowhere and checked it never, so they were last-write-wins. Separately, every form showed a generic "Failed to update X" on failure and discarded the server's message, so even a working conflict looked like a random error; `errorMessage()` now surfaces it. The check stays opt-in — omitting the field skips it, keeping scripted callers working. |
...
| 14.12 | Every control is *operated*, not merely named by a spec | Fixed — `e2e/qa/exhaustive.spec.ts` operates **757 controls across 16 screens**. Kept out of the default run and CI (`npm run test:exhaustive`): it is a build-out tool for after broad UI work, not a per-commit gate, and it costs ten minutes. |
