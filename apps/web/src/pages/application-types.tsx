...
export default function ApplicationTypesPage() {
  ...
  function handleBulkArchive() {
    bulkArchiveMutation.mutate(selectedIds, {
      onSuccess: (result) => {
        toast.success(`Archived ${result.succeeded} application type(s)`);
        setRowSelection({});
        setBulkArchiveOpen(false);
      },
      onError: () => {
        toast.error("Failed to archive application types");
        setBulkArchiveOpen(false);
      },
    });
  }
  ...
  function handleArchive() {
    ...
    archiveMutation.mutate(archivingApplicationType.id, {
      ...
      onError: (error) => {
        toast.error(getApiErrorMessage(error, "Failed to delete application type"));
      },
    });
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[{ label: "Software", href: "/applications" }, { label: "Application Types" }]}
      ...
      <ConfirmDialog
        ...
        title="Archive application types"
        description={`Are you sure you want to archive ${selectedIds.length} application type(s)? This action can be undone later.`}
